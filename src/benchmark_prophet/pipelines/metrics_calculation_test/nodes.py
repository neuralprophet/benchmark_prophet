...
def calculate_metrics(results_cv_with_predictions_refactored, train_fold_results, params):
    ...
    df_pred = results_cv_with_predictions_refactored[f"results_test_pred_{params['input']}_{method}_horizon_{params['n_forecasts']}"]().copy(deep = True)
    ...
    metrics = {
        f"metrics_test_{params['input']}_{method}_horizon_{params['n_forecasts']}": pd.concat(metrices)
    }
